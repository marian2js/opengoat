...
export const agentDirectReporteesCommand: CliCommand = {
  path: ["agent", "direct-reportees"],
  description: "List direct reportees for one agent (one id per line).",
  async run(args, context): Promise<number> {
    ...
    const reportees = await context.service.listDirectReportees(parsed.agentId);
...
function printHelp(output: NodeJS.WritableStream): void {
  output.write("Usage: opengoat agent direct-reportees <agent-id>\n");
}
