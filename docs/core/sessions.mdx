...
Agent defaults for all commands: `config.defaultAgent` / `OPENGOAT_DEFAULT_AGENT` / `goat`.
...
```bash
opengoat agent goat --session q2-planning --message "Create release plan"
opengoat agent goat --session q2-planning --message "Now generate milestones"
```

### Compact long transcripts

```bash
opengoat session compact --agent goat --session q2-planning
```

### Reset context while keeping session key

```bash
opengoat session reset --agent goat --session q2-planning
```
