...
import { readStreamTextDeltas } from "./useBootstrapOrchestrator.js";
...
// ---------------------------------------------------------------------------
// readStreamTextDeltas – SSE stream parsing tests
// ---------------------------------------------------------------------------

/** Helper: create a mock Response from SSE lines. */
...
void test("readStreamTextDeltas: extracts text-delta events from SSE format", async () => {
  ...
  await readStreamTextDeltas(response, (d) => deltas.push(d), controller.signal);

  assert.deepEqual(deltas, ["Hello", " world"]);
});

void test("readStreamTextDeltas: handles empty body", async () => {
...
void test("readStreamTextDeltas: ignores non-text-delta events", async () => {
  const response = mockSSEResponse([
    'data: {"type":"start","messageId":"m1"}',
  ...
  assert.deepEqual(deltas, ["only this"]);
});

void test("readStreamTextDeltas: handles chunked SSE data across multiple reads", async () => {
  const encoder = new TextEncoder();
...
void test("readStreamTextDeltas: skips malformed JSON in data lines", async () => {
  const response = mockSSEResponse([
    "data: {malformed json}",
